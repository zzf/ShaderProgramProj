...
namespace MapMagic
{
	...
	{
		public enum Mode { Custom, RTP, MegaSplat, MicroSplat, CTS };
		...
		public override void OnGUI (GeneratorsAsset gens)
		{
			...
			switch (mode)
			{
				...
				#if __MICROSPLAT__
				case Mode.MicroSplat: layout.Field(ref material, "Material", fieldSize:0.55f); break;
				#endif
			}
			...
			//if (guiLayers)
			{
				...
				if (mode == Mode.MegaSplat || mode == Mode.Custom || mode == Mode.MicroSplat)
				{
		...
		public void OnLayerGUI (Layout layout, bool selected, int num)
		{
			...
			#if __MICROSPLAT__
			if (mode == Mode.MicroSplat && material != null)
			{
				layout.Inset(3);
